...
export function validatePayload(payload: unknown): {
  ...
  if (typeof payload !== "object" || payload === null) {
    return { valid: false, errors: ["Payload invalido."] };
  }
  ...
  if (!isNonEmptyString(candidate.mode)) {
    return { valid: false, errors: ["O modo de criacao e obrigatorio."] };
  }

  if (!nomeProjeto) {
    return { valid: false, errors: ["O nome do projeto e obrigatorio."] };
  }

  if (!isNonEmptyString(candidate.proprietarioProjeto)) {
    return { valid: false, errors: ["O proprietario do projeto e obrigatorio."] };
  }
  ...
  if (candidate.mode === "by_front") {
    ...
    if (missing.length > 0) {
      return {
        valid: false,
        errors: [`Preencha todos os responsaveis obrigatorios. Faltando: ${missing.join(", ")}.`]
      };
    }
  ...
  return { valid: false, errors: ["Modo de criacao desconhecido."] };
}
